...
const cyreneApi = {
  ...
  onPetMirrored: (callback: (mirrored: boolean) => void) => {
    const listener = (_e: unknown, mirrored: boolean) => callback(mirrored);
    ipcRenderer.on(IPC.PET_MIRRORED, listener);
    return () => ipcRenderer.off(IPC.PET_MIRRORED, listener);
  },
...
const settingsApi = {
  ...
  resizeBy: (dx: number, dy: number) => ipcRenderer.send(IPC.SETTINGS_RESIZE_BY, dx, dy),
  ...
  syncNow: () => ipcRenderer.invoke(IPC.SETTINGS_SYNC_NOW),
  syncChatHistory: (days: number) => ipcRenderer.invoke(IPC.SETTINGS_SYNC_CHAT_HISTORY, days),
...
const chatStoreApi = {
  ...
  pin: (id: string, pinned: boolean) =>
    ipcRenderer.invoke(IPC.CHATS_PIN, { id, pinned }),
  reorder: (orderedIds: string[]) =>
    ipcRenderer.invoke(IPC.CHATS_REORDER, orderedIds),
...
// 独立 Game Bot 控制台窗口已删除：「插件」改为直进设置页插件栏
