...
export class Live2DManager {
  ...
  private zoom = 1;
  /** Mirror flip state: when true the model's horizontal scale is negated. */
  ...
  /**
   * Toggle horizontal mirroring. Flips the entire model (including decorations
   * like swings) horizontally. Default facing is right; mirrored faces left.
   */
  applyMirror(mirrored: boolean): void {
    this.mirrored = mirrored;
    if (!this.model) return;
    this.applyScale();
  }

  /** Recompute the model scale from baseScale × zoom, honouring the mirror sign. */
  private applyScale(): void {
    if (!this.model) return;
    const s = this.baseScale * this.zoom;
    // Default: model faces right (positive scale.x)
    // Mirrored: model faces left (negative scale.x, flips everything including decorations)
    this.model.scale.set(this.mirrored ? -s : s, s);
  }
