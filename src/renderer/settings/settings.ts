...
interface SettingsApi {
  ...
  syncChatHistory: (days: number) => Promise<{ success: boolean; count: number }>;
...
if (!window.settings) {
  (window as unknown as { settings: SettingsApi }).settings = {
    ...
    listPlugins: async () => [],
    setPluginEnabled: async () => ({ ok: false, error: "settings api unavailable" }),
    setPluginSettings: async () => ({ ok: false, error: "settings api unavailable" }),
    openPluginWindow: async () => ({ ok: false, error: "settings api unavailable" }),
    relaunchAsAdmin: async () => ({ ok: false }),
...
// 设置窗生产环境用 file:// 加载，绝对路径会解析成 file:///C:/audio/...导致无声，必须用相对路径
...
const petMirroredInput = document.getElementById("pet-mirrored") as HTMLInputElement;
...
const NAV_LABELS: Record<string, { emoji: string; title: string; hint: string }> = {
  ...
  sync: { emoji: "🔄", title: "消息同步", hint: "配置远程服务器的消息同步" },
...
// ── 自定义缩放手柄（透明无边框窗口没有系统原生拖拽边缘） ──
function attachResizeHandles(): void {
  const handles = document.querySelectorAll<HTMLElement>(".settings-resize-handle");
  for (const handle of handles) {
    const mode = handle.dataset.resize; // "right" | "bottom" | "corner"
    if (!mode) continue;
    handle.addEventListener("pointerdown", (e) => {
      ...
      const onMove = (ev: PointerEvent): void => {
        ...
        lastX = ev.clientX;
        lastY = ev.clientY;
        const resizeDx = mode === "bottom" ? 0 : dx;
        const resizeDy = mode === "right" ? 0 : dy;
        if (resizeDx !== 0 || resizeDy !== 0) {
          window.settings?.resizeBy(resizeDx, resizeDy);
        }
      };
...
async function loadGeneralSettings(): Promise<void> {
  try {
    ...
    petMirroredInput.checked = cfg.petMirrored ?? false;
...
petMirroredInput.addEventListener("change", () => {
  void window.settings?.saveGeneral({ petMirrored: petMirroredInput.checked });
  setAppearanceSaveStatus("已应用", "is-ok");
});
...
function switchSection(section: string): void {
  ...
  if (syncPanel) syncPanel.classList.toggle("is-hidden", !isSync);
...
// ===== 功能栏：Game Bot 启用开关（配置与运行在独立控制窗口）=====
void (async () => {
  ...
  const gamebotToggle = document.getElementById("plugin-gamebot-enabled") as HTMLInputElement | null;
  if (!gb || !gamebotToggle) return;
  try {
    const cfg = await gb.getConfig();
    gamebotToggle.checked = Boolean(cfg?.enabled);
  } catch { /* API 不可用时保持默认 UI，不阻断设置页 */ }
  gamebotToggle.addEventListener("change", () => {
    void gb.saveConfig({ enabled: gamebotToggle.checked });
  });
})();
...
(function () {
  ...
  const WARN_KEY = "cyrene.reranker.standard.warned";
  const saved = localStorage.getItem(KEY) || "none";

  /* 首次选择 standard（bge-reranker-base）时的性能提示弹框 */
  const warnOverlay = document.getElementById("reranker-warn-overlay");
  const warnConfirmBtn = document.getElementById("reranker-warn-confirm");
  const warnCancelBtn = document.getElementById("reranker-warn-cancel");
  let warnResolve: ((ok: boolean) => void) | null = null;
  const closeWarning = (ok: boolean) => {
    warnOverlay?.classList.add("is-hidden");
    warnResolve?.(ok);
    warnResolve = null;
  };
  const showStandardWarning = (): Promise<boolean> => new Promise((resolve) => {
    warnResolve = resolve;
    warnOverlay?.classList.remove("is-hidden");
  });
  warnConfirmBtn?.addEventListener("click", () => closeWarning(true));
  warnCancelBtn?.addEventListener("click", () => closeWarning(false));
  ...
  cards.forEach((card) => {
    ...
    card.addEventListener("click", async () => {
      ...
      /* 首次选择 standard 时弹出性能提示，取消则不切换 */
      if (value === "standard" && localStorage.getItem(WARN_KEY) !== "1") {
        const ok = await showStandardWarning();
        if (!ok) return;
...
async function renderSkills(): Promise<void> {
  ...
  function renderSkillRow(s: typeof skills[number]): HTMLDivElement {
    ...
    // 统一 .switch 开关，与全工程其他开关项保持一致
    const { root: toggleRoot, input: toggle } = buildPluginSwitch(s.enabled, async (next) => {
      try {
        await window.settings?.setSkillEnabled?.(s.id, next);
      } catch (err) {
...
// ===== 消息同步设置 =====
const syncEnabledInput = document.getElementById("sync-enabled") as HTMLInputElement | null;
const syncServerUrlInput = document.getElementById("sync-server-url") as HTMLInputElement | null;
const syncPollIntervalInput = document.getElementById("sync-poll-interval") as HTMLInputElement | null;
const syncTimeoutMsInput = document.getElementById("sync-timeout-ms") as HTMLInputElement | null;
const syncHistoryDaysSelect = document.getElementById("sync-history-days") as HTMLSelectElement | null;
const syncSaveBtn = document.getElementById("sync-save-btn") as HTMLButtonElement | null;
const syncNowBtn = document.getElementById("sync-now-btn") as HTMLButtonElement | null;
const syncTestBtn = document.getElementById("sync-test-btn") as HTMLButtonElement | null;
const syncTestDot = document.getElementById("sync-test-dot") as HTMLSpanElement | null;
const syncSaveStatus = document.getElementById("sync-save-status") as HTMLSpanElement | null;
const syncStatusDot = document.getElementById("sync-status-dot") as HTMLSpanElement | null;
const syncStatusLabel = document.getElementById("sync-status-label") as HTMLSpanElement | null;
const syncStatusHint = document.getElementById("sync-status-hint") as HTMLSpanElement | null;

function setSyncStatus(text: string, cls?: string): void {
  if (syncSaveStatus) {
    syncSaveStatus.textContent = text;
    syncSaveStatus.className = "save-status";
    if (cls) syncSaveStatus.classList.add(cls);
  }
}

async function loadSyncPanel(): Promise<void> {
  try {
    const cfg = await window.settings!.getGeneral();
    if (syncEnabledInput) syncEnabledInput.checked = Boolean((cfg as Record<string, unknown>).syncEnabled);
    if (syncServerUrlInput) syncServerUrlInput.value = String((cfg as Record<string, unknown>).syncServerUrl ?? "");
    if (syncPollIntervalInput) syncPollIntervalInput.value = String((cfg as Record<string, unknown>).syncPollInterval ?? 3000);
    if (syncTimeoutMsInput) syncTimeoutMsInput.value = String((cfg as Record<string, unknown>).syncTimeoutMs ?? 5000);
    if (syncHistoryDaysSelect) syncHistoryDaysSelect.value = String((cfg as Record<string, unknown>).syncHistoryDays ?? 7);

    const enabled = Boolean((cfg as Record<string, unknown>).syncEnabled);
    if (syncStatusDot) {
      syncStatusDot.className = `channels-status__dot ${enabled ? "channels-status__dot--online" : "channels-status__dot--offline"}`;
    }
    if (syncStatusLabel) syncStatusLabel.textContent = enabled ? "已启用" : "未启用";
    if (syncStatusHint) syncStatusHint.textContent = enabled ? "随 Agent 启动自动连接" : "启用后将随 Agent 启动自动连接";

    setSyncStatus("等待保存");
  } catch {
    setSyncStatus("读取配置失败", "is-error");
  }
}
...
syncSaveBtn?.addEventListener("click", async () => {
  try {
    setSyncStatus("保存中...");
    await window.settings!.saveGeneral({
      syncEnabled: syncEnabledInput?.checked ?? false,
      syncServerUrl: syncServerUrlInput?.value?.trim() ?? "",
      syncPollInterval: parseInt(syncPollIntervalInput?.value ?? "3000", 10) || 3000,
      syncTimeoutMs: parseInt(syncTimeoutMsInput?.value ?? "5000", 10) || 5000,
      syncHistoryDays: parseInt(syncHistoryDaysSelect?.value ?? "7", 10) || 0,
    });
    setSyncStatus("✅ 已保存");
    const enabled = syncEnabledInput?.checked ?? false;
    if (syncStatusDot) {
      syncStatusDot.className = `channels-status__dot ${enabled ? "channels-status__dot--online" : "channels-status__dot--offline"}`;
    }
    if (syncStatusLabel) syncStatusLabel.textContent = enabled ? "已启用" : "未启用";
    if (syncStatusHint) syncStatusHint.textContent = enabled ? "保存成功，1 分钟后开始同步" : "启用后将随 Agent 启动自动连接";
  } catch {
    setSyncStatus("❌ 保存失败", "is-error");
  }
});

syncNowBtn?.addEventListener("click", async () => {
  try {
    setSyncStatus("正在同步聊天记录...");
    // 先同步聊天记录
    const days = parseInt(syncHistoryDaysSelect?.value ?? "7", 10) || 0;
    const result = await window.settings!.syncChatHistory(days);
    if (result.success) {
      setSyncStatus(`✅ 已同步 ${result.count} 条聊天记录`);
    } else {
      setSyncStatus("⚠️ 聊天记录同步失败", "is-error");
    }
    // 再触发立即同步
    await window.settings!.syncNow();
    if (syncStatusHint) syncStatusHint.textContent = "立即同步已触发，请查看状态";
  } catch {
...
syncTestBtn?.addEventListener("click", async () => {
  ...
  if (!url) {
    setSyncStatus("请先输入服务器地址", "is-error");
    return;
  }
  setSyncStatus("测试中...");
  syncTestBtn.disabled = true;
  syncTestBtn.textContent = "测试中...";
  try {
    // Try to fetch /pull endpoint with a short timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000);
    const response = await fetch(`${url.replace(/\/$/, "")}/pull?since_id=0`, {
      method: "GET",
    ...
    if (response.ok || response.status === 200) {
      if (syncTestDot) {
        syncTestDot.className = "sync-test-dot sync-test-dot--online";
      }
      setSyncStatus("✅ 服务器在线");
    } else {
      if (syncTestDot) {
        syncTestDot.className = "sync-test-dot sync-test-dot--offline";
      }
      setSyncStatus("⚠️ 服务器响应异常");
    }
  } catch {
    if (syncTestDot) {
      syncTestDot.className = "sync-test-dot sync-test-dot--offline";
    }
    setSyncStatus("❌ 服务器不可达", "is-error");
  } finally {
    syncTestBtn.disabled = false;
    syncTestBtn.textContent = "测试";
  }
});
