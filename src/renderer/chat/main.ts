...
function buildRailItem(session: ChatSessionMetaUI): HTMLLIElement {
  ...
  if (session.pinned) li.classList.add("is-pinned");
  ...
  if (session.pinned) titleEl.textContent = `📌 ${titleEl.textContent}`;
  ...
  li.addEventListener("click", async () => {
    if (railDragging) return; // 拖拽结束后不触发点击
  ...
  li.addEventListener("contextmenu", (e) => {
    e.preventDefault();
    e.stopPropagation();
    showRailContextMenu(e.clientX, e.clientY, session);
  });

  // 拖拽排序（mousedown 启动）
  li.addEventListener("mousedown", (e) => {
    if (e.button !== 0) return; // 只左键
    railDragStart(e, session.id, li);
  });
...
// —— 自定义拖拽排序（不依赖 HTML5 drag API，无文本跟随） ——
let railDragging = false;
let railDragFromId: string | null = null;
let railDragGhost: HTMLElement | null = null;
let railDropTargetId: string | null = null;
let railDropPosition: "before" | "after" | null = null;

function railDragStart(e: MouseEvent, sessionId: string, li: HTMLLIElement): void {
  ...
  const onMove = (ev: MouseEvent) => {
    ...
    if (!started) {
      started = true;
      railDragging = true;
      railDragFromId = sessionId;
      li.classList.add("is-dragging");
      document.body.style.userSelect = "none";
      // 创建拖拽幽灵（半透明副本）
      railDragGhost = li.cloneNode(true) as HTMLElement;
      railDragGhost.className = "rail-drag-ghost";
      railDragGhost.style.width = li.offsetWidth + "px";
      railDragGhost.style.left = li.getBoundingClientRect().left + "px";
      railDragGhost.style.top = li.getBoundingClientRect().top + "px";
      document.body.appendChild(railDragGhost);
    }

    // 移动幽灵跟随鼠标
    if (railDragGhost) {
      railDragGhost.style.top = (ev.clientY - li.offsetHeight / 2) + "px";
    }

    // 计算鼠标位于哪个列表项上
    const items = Array.from(chatRailList?.querySelectorAll(".chat__rail-item") ?? []);
    let foundTarget: string | null = null;
    let foundPos: "before" | "after" | null = null;

    for (const item of items) {
      const el = item as HTMLLIElement;
      if (el.dataset.sessionId === sessionId) continue;
      const rect = el.getBoundingClientRect();
      if (ev.clientY >= rect.top && ev.clientY <= rect.bottom) {
        foundTarget = el.dataset.sessionId ?? null;
    ...
    items.forEach(item => {
      ...
      delete (item as HTMLLIElement).dataset.dropPosition;
    });
    if (foundTarget) {
      const targetEl = items.find(el => (el as HTMLLIElement).dataset.sessionId === foundTarget);
      if (targetEl) {
        targetEl.classList.add("is-drag-over");
        if (foundPos) (targetEl as HTMLLIElement).dataset.dropPosition = foundPos;
      }
    }
    railDropTargetId = foundTarget;
    railDropPosition = foundPos;
  };
  ...
  const onUp = async () => {
    ...
    chatRailList?.querySelectorAll(".is-drag-over").forEach(el => {
      el.classList.remove("is-drag-over");
      delete (el as HTMLLIElement).dataset.dropPosition;
    });
    if (railDragGhost) {
      railDragGhost.remove();
      railDragGhost = null;
    }

    if (started && railDragFromId && railDropTargetId && railDragFromId !== railDropTargetId) {
      // 计算新顺序
      const items = Array.from(chatRailList?.querySelectorAll(".chat__rail-item") ?? []);
      const ids = items.map(el => (el as HTMLLIElement).dataset.sessionId!).filter(Boolean);
      const fromIdx = ids.indexOf(railDragFromId);
      if (fromIdx < 0) { railDragging = false; return; }

      const [moved] = ids.splice(fromIdx, 1);
      let toIdx = ids.indexOf(railDropTargetId);
      if (railDropPosition === "after") toIdx += 1;
      ids.splice(toIdx, 0, moved);

      // 持久化新顺序
      await window.chatStore?.reorder(ids);
      // 立即刷新列表（broadcastChanged 跳过发送方窗口，不会自动触发 onChanged）
      void renderRailList();
    }

    // 延迟重置，避免 click 事件触发
    setTimeout(() => { railDragging = false; }, 50);
    railDragFromId = null;
    railDropTargetId = null;
    railDropPosition = null;
  };
...
// —— 右键上下文菜单 ——
function showRailContextMenu(x: number, y: number, session: ChatSessionMetaUI): void {
  ...
  // 更新置顶按钮文字
  const pinBtn = menu.querySelector(".rcm-pin") as HTMLElement | null;
  if (pinBtn) {
    pinBtn.textContent = session.pinned ? "取消置顶" : "置顶";
  }
}

function hideRailContextMenu(): void {
  const menu = document.getElementById("rail-context-menu");
  if (!menu) return;
...
// 点击其他地方关闭菜单
document.addEventListener("click", () => hideRailContextMenu());
document.addEventListener("contextmenu", (e) => {
  const menu = document.getElementById("rail-context-menu");
  if (menu && !menu.contains(e.target as Node)) hideRailContextMenu();
});

// 菜单按钮事件
document.getElementById("rcm-delete")?.addEventListener("click", async () => {
  const menu = document.getElementById("rail-context-menu");
  ...
  deleting = true;
  try {
    await window.chatStore?.delete(sessionId);
    void renderRailList();
    if (sessionId === currentSessionId) {
      const sessions = await window.chatStore?.list();
      if (sessions && sessions.length > 0) {
        await loadSessionTailIntoUI(sessions[0].id);
    ...
    console.warn("[Cyrene Chat] 删除会话失败:", err);
  } finally {
    deleting = false;
  }
});

document.getElementById("rcm-pin")?.addEventListener("click", async () => {
  const menu = document.getElementById("rail-context-menu");
  const sessionId = menu?.dataset.sessionId;
  if (!sessionId) return;
  hideRailContextMenu();
  ...
  if (target) {
    await window.chatStore?.pin(sessionId, !target.pinned);
    void renderRailList();
  }
});
...
function render(preserveScroll = false): void {
  ...
  for (const m of messages) {
    ...
    if (m.source === "synced") {
      ...
      tag.className = "msg__source-tag";
      tag.textContent = "微信聊天";
...
const chatMainEl = document.querySelector(".chat__main") as HTMLElement | null;
let dragCounter = 0;
...
document.addEventListener("dragenter", (e) => {
  ...
  dragCounter += 1;
  // 只在中间聊天区域显示拖放提示
  if (chatMainEl?.contains(e.target as Node)) {
...
document.addEventListener("drop", async (e) => {
  ...
  chatEl?.classList.remove("chat--drag-over");
  // 只在中间聊天区域接受文件拖放
  if (!chatMainEl?.contains(e.target as Node)) return;
...
// ── 右侧状态面板（合并原 sidebar 窗口） ──
const CSP_STATUS_EMOJI: Record<string, string> = {
  陪伴中: "🌸", 思考中: "💭", 工作中: "⚡", 聆听中: "🫧", 提醒中: "🔔", 离线: "💤",
};
const CSP_FEELING_EMOJI: Record<string, string> = {
  平静: "🌿", 开心: "✨", 温柔: "🌸", 激动: "🎉", 撒娇: "🥺",
  担心: "💙", 难过: "💧", 感动: "🥹", 害羞: "🌹",
};

const cspStatusEmoji = document.getElementById("csp-status-emoji") as HTMLElement | null;
const cspStatusLabel = document.getElementById("csp-status-label") as HTMLElement | null;
const cspFeelingEmoji = document.getElementById("csp-feeling-emoji") as HTMLElement | null;
const cspFeelingLabel = document.getElementById("csp-feeling-label") as HTMLElement | null;
const cspModelName = document.getElementById("csp-model-name") as HTMLElement | null;
const cspDot = document.getElementById("csp-dot") as HTMLElement | null;
const cspOnlineLabel = document.getElementById("csp-online-label") as HTMLElement | null;
const cspCallBtn = document.getElementById("csp-call-btn") as HTMLButtonElement | null;
const cspSettingsBtn = document.getElementById("csp-settings-btn") as HTMLButtonElement | null;
const cspGameBtn = document.getElementById("csp-game-btn") as HTMLButtonElement | null;

let cspLifeStatusText: string | null = null;
let cspLifeStatusResting = false;
let cspLatestState: { status: string; feeling: string } | null = null;

function cspFormatLifeStatus(activity: string | null): string {
  if (activity == null) return "休息中";
  return activity.startsWith("在") ? `正${activity}` : `正在${activity}`;
}

function cspApplyRuntimeState(state: { status: string; feeling: string } | null): void {
  cspLatestState = state;
  const status = state?.status ?? "陪伴中";
  const feeling = state?.feeling ?? "平静";
  if (status === "陪伴中" && cspLifeStatusText != null) {
    if (cspStatusEmoji) cspStatusEmoji.textContent = cspLifeStatusResting ? "🌙" : "🌸";
    if (cspStatusLabel) cspStatusLabel.textContent = cspLifeStatusText;
  } else {
    if (cspStatusEmoji) cspStatusEmoji.textContent = CSP_STATUS_EMOJI[status] ?? "💬";
    if (cspStatusLabel) cspStatusLabel.textContent = status;
  }
  if (cspFeelingEmoji) cspFeelingEmoji.textContent = CSP_FEELING_EMOJI[feeling] ?? "🌿";
  if (cspFeelingLabel) cspFeelingLabel.textContent = feeling;
}

function cspApplyModelConfig(config: { connected?: boolean; displayName?: string; shortName?: string; model?: string; runtimeSync?: string } | null): void {
  const connected = Boolean(config?.connected);
  if (cspOnlineLabel) cspOnlineLabel.textContent = connected ? "在线" : "离线";
  if (cspDot) cspDot.classList.toggle("is-offline", !connected);
  // 离线态整个徽章变红（对齐分支侧栏 profile__online.is-offline 样式）
  document.getElementById("csp-online-badge")?.classList.toggle("is-offline", !connected);
  if (cspModelName) cspModelName.textContent = config?.displayName || config?.shortName || config?.model || "未选择模型";
}

async function cspRefreshLifeStatus(): Promise<void> {
  try {
    const activity = await window.lifeStatus?.getCurrentActivity();
    if (activity === undefined) {
      cspLifeStatusText = null;
    } else {
      cspLifeStatusResting = activity == null;
      cspLifeStatusText = cspFormatLifeStatus(activity);
    }
  } catch {
    cspLifeStatusText = null;
  }
  cspApplyRuntimeState(cspLatestState);
}
...
void (async () => {
  try {
    ...
    cspApplyModelConfig(cfg ?? null);
  } catch { cspApplyModelConfig(null); }
  window.modelConfig?.onChanged((cfg) => cspApplyModelConfig(cfg as any));

  try {
    const state = await window.runtimeState?.get();
    cspApplyRuntimeState((state ?? null) as any);
  } catch { cspApplyRuntimeState(null); }
  window.runtimeState?.onChanged((state) => cspApplyRuntimeState(state as any));

  void cspRefreshLifeStatus();
  setInterval(() => void cspRefreshLifeStatus(), 60_000);
})();

// 状态面板按钮
cspCallBtn?.addEventListener("click", () => {
  window.sidebar?.openCall();
});
cspSettingsBtn?.addEventListener("click", () => {
