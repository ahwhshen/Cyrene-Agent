...
const DEFAULT_PER_ROUND_TIMEOUT_MS = 600_000;
const DEFAULT_MAX_CONSECUTIVE_TIMEOUTS = 3;
const DEFAULT_FORCE_SUMMARY_TIMEOUT_MS = 600_000;
...
export async function runTwoPhaseFcLoop(options: TwoPhaseFcOptions): Promise<TwoPhaseFcResult> {
  ...
  // ── TOOL_PHASE 主循环 ──
  // 将角色提醒 + 世界书内容注入到最后一条用户消息中，
  // 确保 14B 等中小模型在注意力最集中处看到身份指令和世界知识。
  if (conversation.length > 0) {
    const lastUserIdx = conversation.map(m => m.role).lastIndexOf("user");
    if (lastUserIdx >= 0) {
      const orig = conversation[lastUserIdx].content ?? "";
      // 从 system prompt 中提取世界书内容（【已激活的世界知识】标记后的部分）
      const wbMarker = "【已激活的世界知识】";
      const wbStart = toolSystemContent.indexOf(wbMarker);
      let wbSection = "";
      if (wbStart >= 0) {
        // 取世界书内容到下一个 --- 分隔符或末尾
        const afterMarker = toolSystemContent.slice(wbStart);
        const nextSep = afterMarker.indexOf("\n\n---\n\n", wbMarker.length);
        wbSection = nextSep >= 0 ? afterMarker.slice(0, nextSep) : afterMarker;
      }
      let hint = "\n\n[系统提示：你是昔涟，请以昔涟的身份和语气直接回复。" +
        "自称用「人家」或「我」，句尾带语气词。" +
        "每次回复要有变化和创造性，不要重复相同的回答，根据用户的具体问题给出不同的回应。";
      if (wbSection) {
        hint += "\n以下是你的世界知识，请自然使用其中的信息回答：\n" + wbSection;
      }
      hint += "]";
  ...
  for (let round = 0; round < maxToolRounds; round++) {
    ...
    // 情况 2：模型没有调工具 → 直接返回 TOOL_PHASE 文本给用户
    // 跳过 SOUL_PHASE 以避免 Ollama peg-native format 错误
    console.log(LOG_PREFIX, "TOOL_PHASE 文本长度:", chat.text.length, "前200字:", chat.text.slice(0, 200));
    onEvent?.({ type: "step_finished", stepName: `tool-round-${round + 1}` });
    ...
    return {
      ...
      soulPhaseReason: "no_tool" as SoulPhaseReason,
    };
  }
...
async function runSoulPhase(args: {
  ...
  /** TOOL_PHASE 生成的文本，SOUL_PHASE 失败时用于降级回复 */
  toolPhaseText?: string;
  ...
  try {
    ...
    // 兜底再失败也别让整个 run 崩掉。
    // 优先使用 TOOL_PHASE 的文本降级回复，其次拼工具结果，最后才用通用错误文案。
    ...
    const fallback = args.toolPhaseText && args.toolPhaseText.trim().length > 0
      ? args.toolPhaseText
      : buildFallbackReply(allToolResults, errReason);
    const textMessageId = `msg-${Date.now()}`;
