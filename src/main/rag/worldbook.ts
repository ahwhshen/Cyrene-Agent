...
export class WorldbookManager {
  ...
  async loadFromDirectory(): Promise<void> {
    ...
    if (this.debug) {
      for (const e of allEntries) {
        console.log(`[Worldbook] entry: ${e.id} | permanent=${e.permanent} | kw=[${e.keywords.join(",")}]`);
      }
    }
  }
  ...
  updateActivation(userText: string, modelText: string): void {
    ...
    if (this.debug) {
      console.log(`[Worldbook/DMAE] userText(${user.length}): "${user.slice(0, 200)}"`);
    }
    ...
    for (const entry of this.entries) {
      ...
      const hit = entry.keywords.some((kw) => user.includes(kw));
      if (this.debug && hit) {
        const matchedKws = entry.keywords.filter((kw) => user.includes(kw));
        console.log(`[Worldbook/DMAE] userHit: ${entry.id} matched=[${matchedKws.join(",")}]`);
      }
      if (hit) {
    ...
    if (this.debug) {
      console.log(`[Worldbook/DMAE] userHit entries: ${userHitEntryIds.size}`, [...userHitEntryIds].join(", "));
    }
