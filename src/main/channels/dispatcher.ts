...
export interface DispatcherDeps {
  ...
  /** 把渠道消息推送到同步服务器（桌面端镜像微信聊天用）。 */
  syncToRemote?: (msg: { uid: string; role: string; content: string; timestamp: number; session_id: string | null }) => void;
}
...
export class ChannelDispatcher {
  ...
  async handleIncoming(msg: IncomingMessage): Promise<OutgoingMessage | null> {
    ...
    // 同步：把微信入站消息推送到同步服务器，供桌面端拉取
    if (msg.channel === "wechat" && this.deps.syncToRemote) {
      try {
        this.deps.syncToRemote({
          uid: `wechat:${msg.senderId}:user:${msg.at.getTime()}`,
          role: "user",
          content: msg.text,
          timestamp: msg.at.getTime(),
          session_id: sessionId,
        });
      } catch (err) {
        console.warn(LOG, "syncToRemote (incoming) 失败:", err);
      }
    }
    ...
    // 同步：把 AI 回复推送到同步服务器，供桌面端拉取
    if (msg.channel === "wechat" && this.deps.syncToRemote) {
      try {
        this.deps.syncToRemote({
          uid: `wechat:${msg.senderId}:assistant:${Date.now()}`,
          role: "assistant",
          content: replyText,
          timestamp: Date.now(),
          session_id: sessionId,
        ...
        console.warn(LOG, "syncToRemote (outgoing) 失败:", err);
      }
    }
...
/** 注入同步回调：把渠道消息推送到同步服务器 */
export function setDispatcherSyncToRemote(
  fn: (msg: { uid: string; role: string; content: string; timestamp: number; session_id: string | null }) => void,
): void {
  channelDispatcher.deps.syncToRemote = fn;
}
