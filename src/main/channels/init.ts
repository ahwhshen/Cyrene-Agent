...
export async function initChannels(): Promise<void> {
  ...
  // 启动所有已注册 adapter（检查 enabled 设置）
  const settings = loadChannelsSettings();
  const skipChannels = new Set<ChannelId>();
  if (!settings.wechat?.enabled) skipChannels.add("wechat");
  if (!settings.feishu?.enabled) skipChannels.add("feishu");
  await channelManager.startAll(skipChannels);
