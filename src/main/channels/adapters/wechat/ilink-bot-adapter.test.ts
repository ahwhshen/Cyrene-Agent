...
describe("ILinkBotAdapter inbound media", () => {
  ...
  it("saves a pending unsupported file when the user replies with save intent within five minutes", async () => {
    ...
    (adapter as any).onMessage = onMessage;
    (adapter as any).client = { sendText };
    (adapter as any).saveInboundMedia = vi.fn(async () => "C:/Users/someone/Desktop/Cyrene 收件箱/archive.zip");
    ...
    expect(sendText).toHaveBeenLastCalledWith(
      "wx-user-1",
      "收好啦，伙伴。人家已经帮你放到桌面的“Cyrene 收件箱”里了：C:/Users/someone/Desktop/Cyrene 收件箱/archive.zip",
      "ctx-text",
    );
    expect(onMessage).not.toHaveBeenCalled();
  });
  ...
  it("saves an unsupported file when it arrives after a save intent", async () => {
    ...
    (adapter as any).onMessage = onMessage;
    (adapter as any).client = { sendText };
    (adapter as any).saveInboundMedia = vi.fn(async () => "C:/Users/someone/Desktop/Cyrene 收件箱/movie.mp4");
    ...
    expect(sendText).toHaveBeenLastCalledWith(
      "wx-user-1",
      "收好啦，伙伴。人家已经帮你放到桌面的“Cyrene 收件箱”里了：C:/Users/someone/Desktop/Cyrene 收件箱/movie.mp4",
      "ctx-video",
    );
    expect(onMessage).not.toHaveBeenCalled();
  });
  ...
  it("saves an analyzable file instead of dispatching it when a save intent is already pending", async () => {
    ...
    (adapter as any).onMessage = onMessage;
    (adapter as any).client = { sendText };
    (adapter as any).saveInboundMedia = vi.fn(async () => "C:/Users/someone/Desktop/Cyrene 收件箱/report.pdf");
