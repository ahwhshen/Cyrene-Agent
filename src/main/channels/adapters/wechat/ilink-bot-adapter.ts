...
export class ILinkBotAdapter implements ChannelAdapter {
  ...
  /** 游标持久化：重启后从上次位置继续拉消息，不丢消息 */
  #cursorPath(): string {
    return path.join(app.getPath("userData"), "weixin", "poll-cursor.txt");
  }

  async #loadCursor(): Promise<string> {
    try {
      return (await fs.readFile(this.#cursorPath(), "utf8")).trim();
    } catch {
      return "";
    }
  }

  async #saveCursor(buf: string): Promise<void> {
    try {
      const dir = path.dirname(this.#cursorPath());
      await fs.mkdir(dir, { recursive: true });
      await fs.writeFile(this.#cursorPath(), buf, "utf8");
    } catch (err) {
      console.warn(LOG_PREFIX, "保存游标失败:", (err as Error).message);
    }
  }
