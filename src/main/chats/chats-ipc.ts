...
export function registerChatsIpc(): void {
  ...
  ipcMain.handle(
    IPC.CHATS_PIN,
    (event, payload: { id: string; pinned: boolean }) => {
      if (!payload?.id) return false;
      const ok = chatsStore.pinSession(payload.id, payload.pinned);
  ...
  ipcMain.handle(
    IPC.CHATS_REORDER,
    (event, orderedIds: string[]) => {
      if (!Array.isArray(orderedIds)) return false;
      const ok = chatsStore.reorderSessions(orderedIds);
      if (ok) broadcastChanged(event.sender);
      return ok;
    },
  );
