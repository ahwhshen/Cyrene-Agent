...
// 货币战争已模块化到 plugins/currency-wars（能力面经 plugin-service.ts 注入），本模块只保留通用脚本引擎。
...
import { loadGameBotSettings, saveGameBotSettings, resolveGameExePath, type GameBotSettings } from "./settings-store";
...
export async function startGameBot(): Promise<{ ok: boolean; error?: string }> {
  ...
  // exe 路径容错：允许填游戏目录，自动解析到目录内的 exe
  const exeResolved = resolveGameExePath(settings.exePath);
  if (exeResolved.error) return { ok: false, error: exeResolved.error };
  const exePath = exeResolved.exe ?? "";
  const resolvedExe = recipe.exe.replace(/\$\{exe_path\}/g, exePath).trim();
  ...
  if (!hasVlm) return { ok: false, error: "未配置可用识别器（VLM）" };

  runningRecipe = settings.activeRecipe;
  runSignal = { aborted: false };
  const tools = buildTools(settings);
  const signal = runSignal;
  const task = runRecipe(recipe, {
    tools,
    vars: { exe_path: exePath, vlm_config: settings.vlm.model },
...
export function initGameBot(): void {
  ...
  // 设置面板最小化/关闭（全屏模式启动后自动最小化设置窗口）
  ipcMain.on("game-bot:minimize", (event) => {
    BrowserWindow.fromWebContents(event.sender)?.minimize();
  });
  ipcMain.on("game-bot:close", () => {
    // 仅关闭设置窗口，不退出应用
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed() && win.getTitle().includes("设置")) {
        win.close();
        break;
      }
    }
