...
import { initPlugins, listPluginsForUi, setPluginEnabled, setPluginSettings, openPluginWindowById, relaunchAsAdmin, disposeAllPlugins } from "./plugins";
import {
...
import { registerGamebotPluginService } from "./game-bot/plugin-service";
...
import { setDispatcherBuildAndRunAgent, setDispatcherSynthesizeTts, setDispatcherBroadcastChat, setDispatcherLoadGeneralSettings, setDispatcherLoadRecentHistory, setDispatcherSyncToRemote } from "./channels/dispatcher";
...
function startSyncClient(): void {
  ...
  if (!fs.existsSync(clientEntry)) {
    console.log("[SyncClient] 客户端同步脚本不存在，跳过:", clientEntry);
    return;
  }

  console.log(`[SyncClient] 启动消息同步客户端... 服务器=${settings.syncServerUrl}`);
  syncClientProcess = spawn("node", [clientEntry], {
    cwd: clientDir,
    stdio: ["ignore", "pipe", "pipe"],
    env: {
      ...process.env,
      SYNC_SERVER_URL: settings.syncServerUrl,
      SYNC_POLL_INTERVAL: String(settings.syncPollInterval),
      SYNC_TIMEOUT_MS: String(settings.syncTimeoutMs),
    },
    shell: true,
  });
  ...
  const timeout = setTimeout(() => {
    if (!connected && syncClientProcess && !syncClientProcess.killed) {
      console.log("[SyncClient] 5 秒内未收到同步服务器响应，跳过消息同步");
    }
  }, 5000);

  syncClientProcess.stdout?.on("data", (data) => {
    ...
    for (const line of raw.split("\n")) {
      ...
      // 检查是否是结构化同步数据
      if (msg.startsWith("__SYNC_DATA__:")) {
        try {
          const json = JSON.parse(msg.slice("__SYNC_DATA__:".length));
          if (json.type === "pull" && Array.isArray(json.messages)) {
            handleSyncedMessages(json.messages);
          }
        } catch {
          console.warn("[SyncClient] 解析同步数据失败:", msg.slice(0, 200));
        }
        continue;
      }
      ...
      if (msg.includes("✅") || msg.includes("数据库已就绪")) {
        connected = true;
  ...
  syncClientProcess.stderr?.on("data", (data) => {
    const msg = data.toString().trim();
    if (msg) console.warn(`[SyncClient] ${msg}`);
  });

  syncClientProcess.on("error", (err) => {
    console.warn("[SyncClient] 进程错误:", err.message);
    clearTimeout(timeout);
  });

  syncClientProcess.on("exit", (code) => {
    console.log(`[SyncClient] 进程退出，code=${code}`);
    syncClientProcess = null;
    clearTimeout(timeout);
  });
}

function stopSyncClient(): void {
  if (syncClientProcess && !syncClientProcess.killed) {
    console.log("[SyncClient] 正在关闭同步客户端...");
    syncClientProcess.kill();
    syncClientProcess = null;
  }
}

function restartSyncClient(): void {
  // Clear any pending sync start timer
  if (syncStartTimer) {
  ...
  const settings = loadGeneralSettings();
  if (settings.syncEnabled) {
    // Delay sync start by 1 minute after saving config
    console.log("[SyncClient] 配置已保存，1 分钟后启动同步...");
...
/** 显示 Windows 桌面通知（标题固定为昔涟Cyrene，内容截取前 20 字） */
function showDesktopNotification(body: string): void {
  const truncated = body.length > 20 ? body.slice(0, 20) + "…" : body;
  try {
    ...
    console.warn("[Notification] 发送通知失败:", (err as Error).message);
  }
}

/**
 * 把已处理的远程消息 uid 写入同步客户端的 merge-queue，
 * 客户端下个 tick 会标记 merged，避免去重/已存在的消息被反复重发。
 */
function enqueueMergedUids(uids: string[]): void {
  if (uids.length === 0) return;
  try {
    const clientDir = path.join(__dirname, "..", "..", "..", "client");
    const queuePath = path.join(clientDir, "merge-queue.json");
    let existing: string[] = [];
    ...
    const merged = Array.from(new Set([...existing, ...uids]));
    fs.writeFileSync(queuePath, JSON.stringify(merged), "utf8");
  } catch (err) {
    console.warn("[SyncClient] 写入 merge-queue 失败:", (err as Error).message);
  }
}

/** 处理同步客户端拉取到的消息，合并到本地聊天存储 */
function handleSyncedMessages(messages: Array<{ uid: string; role: string; content: string; timestamp: number; session_id: string | null }>): void {
  try {
    const count = chatsStore.mergeSyncedMessages(messages);
    // 无论新插入还是被去重跳过，这些 uid 在聊天存储里都已存在，全部标记已合并
    enqueueMergedUids(messages.map((m) => m.uid).filter((uid) => typeof uid === "string" && uid.length > 0));
    if (count > 0) {
      console.log(`[SyncClient] 已合并 ${count} 条远程消息到聊天存储`);
      // 通知聊天窗口刷新（如果打开的话）
      if (chatWindow && !chatWindow.isDestroyed()) {
        chatWindow.webContents.send("chats:sync-updated");
      }
      // Windows 桌面通知：取最后一条 agent 消息的内容（用户自己发的消息不通知）
      const lastAgentMsg = [...messages].reverse().find((m) => {
        const r = (m.role ?? "").toLowerCase();
        return (r === "assistant" || r === "model") && m.content;
      });
      if (lastAgentMsg) {
        showDesktopNotification(lastAgentMsg.content);
      }
    }
  } catch (err) {
    console.warn("[SyncClient] 合并同步消息失败:", (err as Error).message);
  }
}

/** 将本地消息写入 outbox，供同步客户端读取并推送到服务器 */
function queueLocalMessageForSync(msg: { uid: string; role: string; content: string; timestamp: number; session_id: string | null }): void {
  const clientDir = path.join(__dirname, "..", "..", "..", "client");
  const outboxPath = path.join(clientDir, "outbox.json");
  try {
    ...
    existing.push(msg);
    fs.writeFileSync(outboxPath, JSON.stringify(existing, null, 2), "utf-8");
    console.log(`[SyncClient] 本地消息已加入 outbox: ${msg.uid}`);
  } catch (err) {
    console.warn("[SyncClient] 写入 outbox 失败:", (err as Error).message);
  }
}
...
interface GeneralSettings {
  ...
  /** 🔄 消息同步：是否启用 */
  syncEnabled: boolean;
  /** 🔄 消息同步：服务器地址（如 http://your-server:8080） */
  syncServerUrl: string;
  /** 🔄 消息同步：轮询间隔（毫秒），3000~60000 */
  syncPollInterval: number;
  /** 🔄 消息同步：请求超时（毫秒），1000~30000 */
  syncTimeoutMs: number;
  /** 🔄 消息同步：聊天记录同步时间范围（天数），0 表示所有 */
...
const CHAT_REQUEST_TIMEOUT_MS = 600000; // FC 总预算：20 轮 × 14B 模型 ~20-30s 需 600s 余量
...
function getAppIconPath(icon: UiIcon): string {
  ...
  const baseName = (preset?.fileName ?? "cyrene-sun.png").replace(/\.png$/i, "");
  const dir = path.join(__dirname, "..", "..", "..", "assets", "icon-presets");
  // Windows 任务栏/Alt+Tab 对多尺寸 .ico 渲染最可靠，纯大尺寸 PNG 会退回 Electron 默认图标
...
function normalizeGeneralSettings(input: Partial<GeneralSettings> | null | undefined): GeneralSettings {
  ...
  return {
    ...
    petMirrored: input?.petMirrored === undefined ? DEFAULT_GENERAL_SETTINGS.petMirrored : Boolean(input.petMirrored),
    ...
    // 消息同步
    syncEnabled: input?.syncEnabled === undefined ? DEFAULT_GENERAL_SETTINGS.syncEnabled : Boolean(input.syncEnabled),
    syncServerUrl: typeof input?.syncServerUrl === "string" ? input.syncServerUrl : DEFAULT_GENERAL_SETTINGS.syncServerUrl,
    syncPollInterval: typeof input?.syncPollInterval === "number"
      ? Math.max(1000, Math.min(120000, Math.round(input.syncPollInterval)))
      : DEFAULT_GENERAL_SETTINGS.syncPollInterval,
    syncTimeoutMs: typeof input?.syncTimeoutMs === "number"
      ? Math.max(1000, Math.min(30000, Math.round(input.syncTimeoutMs)))
      : DEFAULT_GENERAL_SETTINGS.syncTimeoutMs,
    syncHistoryDays: typeof input?.syncHistoryDays === "number"
      ? Math.max(0, Math.round(input.syncHistoryDays))
      : DEFAULT_GENERAL_SETTINGS.syncHistoryDays,
  };
}
...
function applyGeneralSettings(settings: GeneralSettings, previous?: GeneralSettings): void {
  ...
  // 镜像开关变化时推送到桌宠窗口（气泡居中且对称，随模型镜像自动保持对齐）
  if (!previous || previous.petMirrored !== settings.petMirrored) {
    sendToLive2DWindow(IPC.PET_MIRRORED, settings.petMirrored);
  }
}
...
function logWorldbookInjection(alwaysOnContext: string, systemContent: string): void {
  const activeMarker = "【已激活的世界知识】";
  const permanentMarker = "【常驻背景】";
  const hasActive = alwaysOnContext && alwaysOnContext.includes(activeMarker);
  const hasPermanent = alwaysOnContext && alwaysOnContext.includes(permanentMarker);
  if (hasActive || hasPermanent) {
    const marker = hasPermanent ? permanentMarker : activeMarker;
    ...
    console.log(`[Worldbook/Diag] 常驻条目: ${hasPermanent ? "✓ 已注入" : "✗ 无"}`);
    console.log(`[Worldbook/Diag] 动态条目: ${hasActive ? "✓ 已注入" : "✗ 无"}`);
...
async function commitLocalProactiveMessage(input: ProactiveCommitInput): Promise<ProactiveCommitResult> {
  ...
  // 将主动消息加入同步 outbox
  queueLocalMessageForSync({
    uid: `desktop:${session.id}:assistant:${at}`,
    role: "assistant",
    content: input.text,
    timestamp: at,
  ...
  if (displayDecision.allowed && mainWindow && !mainWindow.isDestroyed()) {
    ...
    // Windows 桌面通知
    showDesktopNotification(input.text);
  }
  return { kind: "committed" };
}

/** 检查桌面端是否在线（通过同步服务器的 /presence 端点） */
async function isDesktopOnline(syncServerUrl: string): Promise<boolean> {
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 3000);
    const res = await fetch(`${syncServerUrl}/presence`, { signal: controller.signal });
    clearTimeout(timer);
...
async function commitSelectedProactiveMessage(input: ProactiveCommitInput): Promise<ProactiveCommitResult> {
  ...
  const result = await routeProactiveDelivery(target, {
    ...
    commitChannel: async (channel) => {
      // 桌面端在线时，跳过渠道主动消息（桌面端会直接显示）
      if (await isDesktopOnline(settings.syncServerUrl)) {
        console.log(`[Proactive] 桌面端在线，跳过 ${channel} 渠道主动消息`);
        return { kind: "cancelled", reason: "desktop_online" };
      }
...
/** 独立 Game Bot 控制台窗口已删除：「插件」按钮改为直进设置页插件栏。 */
...
ipcMain.handle(IPC.CHAT_SEND_MESSAGE, async (_event, messages: unknown) => {
  ...
  try {
    ...
    const lastUserMsg = msgList.filter((m: any) => m?.role === "user").at(-1);
    if (lastUserMsg && typeof lastUserMsg.content === "string") {
      queueLocalMessageForSync({
        uid: `desktop:${activeChatSessionId ?? "default"}:user:${Date.now()}`,
        role: "user",
    ...
    if (reply?.reply) {
      queueLocalMessageForSync({
        uid: `desktop:${activeChatSessionId ?? "default"}:assistant:${Date.now()}`,
        role: "assistant",
        content: reply.reply,
        timestamp: Date.now(),
        session_id: activeChatSessionId,
...
// 透明无边框窗口没有系统拖拽边缘，设置页用自定义手柄发增量过来，这里限幅后 setSize
ipcMain.on(IPC.SETTINGS_RESIZE_BY, (_event, dx: number, dy: number) => {
  if (!settingsWindow || settingsWindow.isDestroyed()) return;
  const [width, height] = settingsWindow.getSize();
  const minWidth = 920;
  const minHeight = 580;
  const display = screen.getPrimaryDisplay().workArea;
  const maxWidth = Math.max(minWidth, display.width);
  const maxHeight = Math.max(minHeight, display.height);
  settingsWindow.setSize(
    Math.min(maxWidth, Math.max(minWidth, width + Math.round(Number(dx) || 0))),
    Math.min(maxHeight, Math.max(minHeight, height + Math.round(Number(dy) || 0))),
  );
});
...
ipcMain.handle(IPC.SETTINGS_SAVE_GENERAL, (_event, settings: Partial<GeneralSettings>) => {
  ...
  // 消息同步动态启停：设置变更后立即生效
  if ("syncEnabled" in settings || "syncServerUrl" in settings || "syncPollInterval" in settings || "syncTimeoutMs" in settings) {
    restartSyncClient();
  }
  return saved;
});
...
// 立即触发同步（跳过 1 分钟延迟）
ipcMain.handle(IPC.SETTINGS_SYNC_NOW, () => {
  console.log("[SyncClient] 用户触发立即同步...");
...
// 同步聊天记录到服务器（按时间范围筛选）
ipcMain.handle(IPC.SETTINGS_SYNC_CHAT_HISTORY, async (_event, days: number) => {
  try {
    console.log(`[SyncClient] 同步聊天记录，时间范围: ${days === 0 ? "所有" : days + " 天"}`);
    
    // 计算时间戳阈值
    const now = Date.now();
    const threshold = days === 0 ? 0 : now - days * 24 * 60 * 60 * 1000;
    ...
    for (const sessionMeta of sessions) {
      const session = chatsStore.getSession(sessionMeta.id);
      if (!session || !session.messages) continue;
      
      // 过滤消息并写入 outbox
      for (const msg of session.messages) {
        if (msg.at < threshold) continue;
        if (!msg.content || msg.content.trim() === "") continue;
        
        // 角色映射：本地用 model，同步协议用 assistant
        const role = msg.role === "model" ? "assistant" : msg.role;
        
        queueLocalMessageForSync({
          uid: `desktop:${session.id}:${role}:${msg.at}`,
          role: role,
          content: msg.content,
          timestamp: msg.at,
          session_id: session.id,
    ...
    console.log(`[SyncClient] 已写入 ${count} 条聊天记录到 outbox`);
    ...
    console.warn("[SyncClient] 同步聊天记录失败:", (err as Error).message);
...
// ── 插件系统（外部扩展通道；核心功能不走此通道） ──────────────
ipcMain.handle(IPC.PLUGIN_LIST, () => {
  return listPluginsForUi();
});

ipcMain.handle(IPC.PLUGIN_SET_ENABLED, (_event, payload: unknown) => {
  const p = payload as { id?: string; enabled?: boolean };
  if (!p.id) return { ok: false, error: "missing plugin id" };
  const result = setPluginEnabled(p.id, p.enabled !== false);
  console.log("[Plugins] " + p.id + " enabled=" + (p.enabled !== false) + (result.error ? ` (${result.error})` : ""));
  return result;
});

ipcMain.handle(IPC.PLUGIN_SET_SETTINGS, (_event, payload: unknown) => {
  const p = payload as { id?: string; settings?: Record<string, unknown> };
  if (!p.id) return { ok: false, error: "missing plugin id" };
  return setPluginSettings(p.id, p.settings ?? {});
});

ipcMain.handle(IPC.PLUGIN_OPEN_WINDOW, (_event, payload: unknown) => {
  const p = payload as { id?: string };
  if (!p.id) return { ok: false, error: "missing plugin id" };
  return openPluginWindowById(p.id);
});

ipcMain.handle(IPC.PLUGIN_RELAUNCH_ADMIN, () => {
  relaunchAsAdmin();
  return { ok: true };
});
...
app.setAppUserModelId("com.cyrene.app");

// 单实例锁：已有实例在运行时，重复启动（如再次双击启动脚本）不再开新进程，
// 而是由已运行的实例打开并聚焦聊天主界面
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle(IPC.TTS_SYNTHESIZE_CACHED_GPTSOVITS, async (_event, payload: GptsovitsSynthesizeRequest & {
    ...
    // GPT-SoVITS 不支持 mp3，自动回退到 wav
    const rawFormat = payload.format ?? "wav";
    const format: "wav" | "mp3" = rawFormat === "mp3" ? "wav" : rawFormat;
  ...
  // 插件系统：扫描 appPath/plugins + userData/plugins，加载启用的插件（增量扩展通道，核心功能不走此通道）
  // gamebot 宿主服务必须在 initPlugins 之前注册（货币战争插件声明 uses:["gamebot"]，并携带存量配置迁移）
  registerGamebotPluginService();
  initPlugins({
    appPath: app.getAppPath(),
    userDataPath: app.getPath("userData"),
    pluginWindowPreloadPath: path.join(__dirname, "..", "..", "preload", "preload", "plugin-window.js"),
    appIconPath: getCurrentAppIconPath(),
  });
  try {
    loadMusicCompanionHost(
  ...
  // 注入同步回调：微信消息推送到同步服务器，桌面端拉取显示在对话列表
  setDispatcherSyncToRemote((msg) => {
    queueLocalMessageForSync(msg);
  });
  ...
  setProactiveReplyRunner(async ({ text, images }) => {
    ...
    try {
      ...
      // 将手机回复和 AI 回复加入同步 outbox
      queueLocalMessageForSync({
        uid: `mobile:${sessionId}:user:${userAtMs}`,
        role: "user",
        content: text,
        timestamp: userAtMs,
      ...
      queueLocalMessageForSync({
        uid: `mobile:${sessionId}:assistant:${assistantAtMs}`,
        role: "assistant",
        content: cleanReply,
        timestamp: assistantAtMs,
        session_id: sessionId,
      });
  ...
  createWindow();
  createChatWindow();
  // sidebar 已合并进聊天窗口右侧面板，不再创建独立窗口
  if (generalSettings.tasksVisible) createTasksWindow();
  createTray();
  // 启动消息同步客户端（非阻塞，5 秒无响应则跳过）
  startSyncClient();
